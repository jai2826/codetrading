...
const page = ({ blogs }) => {
  ...
  return (
    <>
      <Head>
        <title>Technews - Get tech news on the go</title>
        <meta name="description" content="Get tech news on the go" />
        ...
        <div className="2xl:container mx-auto px-4 lg:py-10  2xl:px-8 border-2 my-6 border-gray-100">
          <div className=" flex flex-wrap mb-20 md:flex-col">
            <div className="lg:w-1/2 w-full mb-6 lg:mb-0">
              <h1 className=" font-medium mb-2">Get tech news on the go</h1>
            </div>
                  ...
                  return (
                    <Link key={item.id} href={`/Blog/${item.attributes.slug}`}>
                      <div className="bg-white flex flex-col overflow-hidden  rounded-lg shadow-lg w-full sm:w-1/3 md:w-1/4 xl:w-w22  p-2 m-3  cursor-pointer">
                        <div className="h-56 overflow-hidden">{item.attributes.image.data && <img className="object-fill object-center h-full rounded-lg" src={item.attributes.image.data.attributes.url} width={1200} height={1000} alt="" />}</div>
...
export async function getServerSideProps(context) {
  ...
  let a = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/blogs?filters[categories][name]=Technews&sort=updatedAt%3Adesc&populate=*`, { headers: headers })
