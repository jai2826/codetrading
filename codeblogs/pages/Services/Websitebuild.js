          ...
          {newplans.map((item) => {
            return (<div key={item.id} className="p-4 xl:w-1/4 max-h-128 md:w-1/2 w-full">
              <div className="h-full p-6 rounded-lg border-2 border-gray-300 flex flex-col  overflow-hidden">
                ...
                {(item.attributes.footer != null) && <p className="text-xs text-gray-500 mt-3">Maintenance Charges: ₹{item.attributes.footer}</p>}
              </div>
            </div>)
          })}
...
export async function getServerSideProps(context) {
  ...
  let a = await fetch(`${process.env.NEXT_PUBLIC_STRAPI_URL}/api/web-buildings?populate=*`, { headers: headers })
