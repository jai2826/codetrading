...
const Launchpad = ({ blogs  })=>{
    let newblogs = [];
    {
        blogs.data.map((item)=>{
            item.attributes.categories.data.map((item2)=>{
                if (item2.attributes.name === "Launchpad") newblogs.push(item);
            });
        });
    }
    newblogs.sort((a, b)=>{
...
var __webpack_exports__ = __webpack_require__.X(0, [686,397,676,664,675], () => (__webpack_exec__(2614)));
